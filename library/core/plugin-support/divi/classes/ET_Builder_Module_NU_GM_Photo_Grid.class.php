...
class ET_Builder_Module_NU_GM_Photo_Grid extends ET_Builder_Module {
  public static $module_slug = 'et_pb_nu_gm_photo_grid';

  function init() {
    $this->name               = esc_html__( 'Photo Grid', 'nu_gm' );
    $this->slug               = $this->get_module_slug();
    $this->fullwidth          = true;
    $this->custom_css_tab     = false;
    $this->child_slug         = 'et_pb_nu_gm_photo_grid_item';
    $this->child_item_text    = esc_html__( 'Photo Grid Item', 'nu_gm' );
  ...
  function shortcode_callback( $atts, $content = null, $function_name ) {
    ...
    $inner_content           = preg_replace_callback(
                                '|(<a class="photo\-box\-img\-link\sphoto\-box\-fancybox\-img\-link\sfancybox"[^>]+>)(.*)(</a>)|siU',
                                function( $matches ) use ( $enable_lightbox, $lightbox_group_id ) {
                                  ...
                                  if ( $enable_lightbox ) {
                                    ...
                                    $inner   = preg_replace( '|<a class="photo\-box\-img\-link"[^>]+>|', '', $inner );
                                  } else {
                                    $start_a = '';
                                    if ( !preg_match( '|<a class="photo\-box\-img\-link|', $inner ) )
                                      $end_a = '';
                                  }
    ...
    $output  = '<section'.$module_id.$aria_label.' class="clearfix contain-1120 '.preg_replace('|_|', '-', $this->slug).'">';
      ...
      $output .= '<div class="photo-grid">'.$inner_content.'</div>';
    $output .= '</section>';

    return $output;
  }
}
new ET_Builder_Module_NU_GM_Photo_Grid;
